...
var ALAPTOMB = ObjektumFeltoltese();
function PontszerzoHelyezes(objektumTomb) {
    var counter = 0;
...
function EremStatisztika(objektumTomb) {
    var eremObjektum = {
        arany: 0,
        ezust: 0,
        bronz: 0,
...
function PontokOsszege(objektumTomb) {
    var counter = 0;
    for (var i = 0; i < objektumTomb.length; i++) {
        if (objektumTomb[i].helyezes == 1) {
            counter += 7;
...
function TornaVUszas(objektumTomb) {
    var torna = 0;
    var uszas = 0;
    for (var i = 0; i < objektumTomb.length; i++) {
...
function UjTomb(objektumTomb) {
    ...
    for (var i = 0; i < objektumTomb.length; i++) {
        ...
        if (objektumTomb[i].sportNeve == "kajakkenu") {
            ...
            szoveg = objektumTomb[i].helyezes + " " + objektumTomb[i].letszam + " " + objektumTomb[i].sportNeve + " " + objektumTomb[i].versenyNeve;
        }
        javitottTomb.push(szoveg);
    }
    return javitottTomb;
}
function LegtobbPontSzerzo(objektumTomb) {
    var pontszerzoTomb = [];
    for (var i = 0; i < objektumTomb.length; i++) {
        if (objektumTomb[i].helyezes > 0 && objektumTomb[i].helyezes < 7) {
    ...
    var max = pontszerzoTomb[0].letszam;
    var eredmeny = {
    ...
    for (var i = 0; i < pontszerzoTomb.length; i++) {
...
console.log("Pontszerz\u0151 helyez\u00E9sek sz\u00E1ma: ".concat(PontszerzoHelyezes(ALAPTOMB)));
console.log(EremStatisztika(ALAPTOMB));
console.log("Olimpiai pontok sz\u00E1ma: ".concat(PontokOsszege(ALAPTOMB)));
